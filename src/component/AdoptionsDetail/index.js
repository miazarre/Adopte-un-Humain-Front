...
const AdoptionsDetail = ({ isLogged }) => {
  ...
  const getAnimal = async () => {
    try {
      ...
      const responsebis = await axios.get(
        `${baseUrl}/animal/${idAnimal.id}/tag`,
        { headers: { Authorization: `Bearer ${newToken}` } },
      );
      ...
      setMessage(
        "Il y a eu un problème avec le serveur au moment de récupérer les informations sur l'animal.",
      );
      console.log(error);
    }
  };
  ...
  const getAdoptions = async () => {
    try {
      ...
      setMessage(
        "Il y a eu un problème avec le serveur au moment de récupérer les informations sur les adoptions.",
      );
      console.log(error);
    }
  };
  ...
  return (
    <div className="adoptionsdetail">
      {isLogged ? (
        <>
          {message != "" && (
            <p className="adoptionsdetail--message">
              {message} <RxCrossCircled onClick={(e) => setMessage("")} />
            </p>
          )}
          <div className="adoptionsdetail__animal-details">
            <p className="adoptionsdetail__animal-details--name">
              {animal.name}
            </p>
            <div
              className="adoptionsdetail__image"
              style={{
            ...
            <div className="adoptionsdetail__tags-container">
              {tags.map((tag) => {
                return (
                  <span
                    key={tag.tag_name}
                    className="adoptionsdetail__tags-container--tags"
                  >
                    {tag.tag_name}
                  </span>
                );
