...
const Preferences = ({ isLogged, user }) => {
  ...
  const getTagId = async (selectedOption) => {
    ...
    let foundTag = response.data.filter(
      (tag) => tag.name.toLowerCase() === selectedOption.value
    );
    return foundTag;
  };
  ...
  const addingTag = async (selectedOption) => {
    try {
      ...
      const existingTag = tags.find(
        (tag) => tag.tag_name.toLowerCase() === selectedOption.value
      );
        ...
        <>
              ...
              {tags.map((tag) => {
                if (tag.priority === true) {
                  return (
                    <span key={tag.tag_id}>
                      {tag.tag_name}{" "}
                      <RxCrossCircled onClick={deletingTag(tag.tag_id)} />
                    </span>
                  );
                }
              })}
            ...
            <div className="preference__actual-profil--tags">
              {tags.map((tag) => {
                if (tag.priority === false) {
                  return <span key={tag.tag_id}>{tag.tag_name}</span>;
                }
              })}
            </div>
          </div>
          <form className="preference__form-container">
            ...
            <div className="preference__form-container--formdiv">
              ...
              />
              <Select
                options={optionsTempsBallade}
                placeholder="Temps de ballades par jour"
                className="preference__form-container--select"
                styles={customStyles}
                onChange={handleChange}
              />
              <Select
                options={optionsTempsSolo}
                placeholder="Heures d'absence par jour"
                className="preference__form-container--select"
                styles={customStyles}
                onChange={handleChange}
              />
