  ...
  const getMatching = async () => {
    try {
      const response = await axios.get(
        `${baseUrl}/user/${user.id}/matching/${animal.id}`,
  ...
  return (
    <div className="animal-card__card">
      ...
      <div className="animal-card__card--match">
        <p className="animal-card__card--points">
          {matching.count} points communs -{" "}
          {matching.pourcentageDone ? matching.pourcentageDone : "0"}%
        </p>
        {matching.count != "" && (
          <PieChart
      ...
      <Link
        to={`/trombinoscope/${animal.id}`}
        className="animal-card__card--bouton"
      >
        <span>Profil</span>
      </Link>
    </div>
