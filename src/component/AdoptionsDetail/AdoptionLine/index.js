...
const AdoptionLine = ({ adoption, getAdoptions, setMessage }) => {
  ...
  const getUser = async () => {
    try {
      const response = await axios.get(
        `${baseUrl}/admin/user/${adoption.user_id}`,
      ...
      setMessage(
        "Il y a eu un problème au moment de la récupération des informations sur l'utilisateur.",
      );
      console.log(error);
    }
  };
  ...
  const handleSaveComment = async () => {
    try {
      ...
      setMessage(
        "Il y a eu un problème au moment du contact avec le serveur pour changer le commentaire de l'adoption.",
      );
      console.log(error);
    }
  };
  ...
  const handleStatusChange = async (e) => {
    try {
      ...
      setMessage(
        "Il y a eu un problème au moment du contact avec le serveur pour changer le statut de l'adoption.",
      );
      console.log(error);
    }
  };
  ...
  const getMatching = async () => {
    try {
      const response = await axios.get(
        `${baseUrl}/user/${adoption.user_id}/matching/${param.id}`,
        { headers: { Authorization: `Bearer ${newToken}` } },
      );
  ...
  return (
    <div className="adoptionLine__container">
      ...
      <p className="adoptionLine__user--phone">
        ...
        <div className="adoptionLine__bigdetails--modal">
          <div className="adoptionLine__bigdetails">
            <div className="adoptionLine__bigdetails--title--container">
              <p className="adoptionLine__bigdetails--title">
            ...
            <div className="adoptionLine__bigdetails--body-part">
              <div className="adoptionLine__bigdetails--body-part--user">
                <h3>Utilisateur</h3>
                <p className="adoptionLine__bigdetails--body-part--user-name">
                  {user.firstname} {user.lastname}
                </p>
                <p>
                  <BsFillTelephoneFill /> {user.phone}
                </p>
                <p className="adoptionLine__bigdetails--body-part--address">
                  {user.address} {user.postal_code} {user.city} {user.country}
                </p>
                <p>{matching.count} tags en commun</p>
                <p className="adoptionLine__bigdetails--body-part--match">
                  {matching.pourcentageDone}% de match !
                </p>
